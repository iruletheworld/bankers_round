...
但是此方法和常用的“四舍五入”法差别较大（主要是逻辑判断要多一些），而且常用的表格软件的默认留位法和它不同，譬如Excel的默认公式（但VBA的默认是），故此在此作些讨论。
...
虽然 Python 3 自带的“round()”函数所使用的方法就是四舍六入五留双，由于小数在计算机中永远只是近似值，得出的结果还是可能会出问题的。可靠的做法是用Python 自带的“decimal”模块来实现。在此给出一个例子。
...
但是此方法和常用的“四捨五入”法差別較大（主要是邏輯判斷要多一些），而且常用的表格軟件的默認留位法和它不同，譬如Excel的默認公式（但VBA的默認是），故此在此作些討論。
...
雖然 Python 3 自帶的“round()”函數所使用的方法就是四捨六入五留雙，由於小數在計算機中永遠只是近似值，得出的結果還是可能會出問題的。可靠的做法是用Python 自帶的“decimal”模塊來實現。在此給出一個例子。
