...
因为 Excel VBA 中默认的留位方法就是四舍六入五留双，用户可能会认为只需要直接调用即可。但是，其实这样做在引用单元个时还是可能会出错（直接给数值一般不会出错，但 Excel 的强大之处在於单元格的调用，不用就没意思了）。
...
因爲 Excel VBA 中默認的留位方法就是四捨六入五留雙，用戶可能會認爲只需要直接調用即可。但是，其實這樣做在引用單元個時還是可能會出錯（直接給數值一般不會出錯，但 Excel 的強大之處在於單元格的調用，不用就沒意思了）。
